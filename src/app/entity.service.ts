...
export class EntityService {  
  ...
  getAllEntity(url: string): Observable < IEntity[] > {  
      return this.http.get < IEntity[] > (url).pipe(catchError(this.handleError));  
  }  
  // insert new entity details    
  addEntity(url: string, entity: IEntity): Observable < any > {  
    return this.http.post(url, entity, httpPostOptions).pipe(catchError(this.handleError));  
  }  
  // update entity details    
  updateEntity(url: string, id: number, entity: IEntity): Observable < any > {  
      const newurl = `${url}?id=${id}`;
      return this.http.put(newurl, entity, httpPostOptions).pipe(catchError(this.handleError));  
  }  
