...
export class StateService {  
  ...
  getAllState(url: string): Observable < IState[] > {  
      return this.http.get < IState[] > (url).pipe(catchError(this.handleError));  
  }  
  // insert new state details    
  addState(url: string, state: IState): Observable < any > {  
    return this.http.post(url, state, httpPostOptions).pipe(catchError(this.handleError));  
  }  
  // update state details    
  updateState(url: string, id: number, state: IState): Observable < any > {  
      const newurl = `${url}?id=${id}`;
      return this.http.put(newurl, state, httpPostOptions).pipe(catchError(this.handleError));  
  }  
