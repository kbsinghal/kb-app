import {  
  ...
  export class UserService {  
    ...
    addUser(url: string, user: IUser): Observable < any > {  
      return this.http.post(url, user, httpPostOptions).pipe(catchError(this.handleError));  
    }  
    // update user details    
    updateUser(url: string, id: number, user: IUser): Observable < any > {  
        const newurl = `${url}?id=${id}`;
        return this.http.put(newurl, user, httpPostOptions).pipe(catchError(this.handleError));  
    }  
